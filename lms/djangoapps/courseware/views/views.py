...
@login_required
@xframe_options_exempt
def render_xblock_particular(request, usage_key_string):
    """
    Returns an HttpResponse with HTML content for a particular xBlock specified by usage_key.
    This is similar to render_xblock but designed for rendering a specific particular block
    that is passed in the URL, suitable for iframe embedding.
    ...
    usage_key = usage_key.replace(course_key=modulestore().fill_in_run(usage_key.course_key))
    course_key = usage_key.course_key
    print("11111111111111 : course_key",course_key)
    # Gathering metrics to make performance measurements easier.
    set_custom_attributes_for_course_key(course_key)
    set_custom_attribute('usage_key', usage_key_string)
    set_custom_attribute('block_type', usage_key.block_type)
    ...
    check_if_enrolled = request.GET.get('check_if_enrolled', '1') == '1'
    staff_access = has_access(request.user, 'staff', course_key)
    print("11111111111111 : staff_access",staff_access)
    ...
    with branch_setting, modulestore().bulk_operations(course_key):
        ...
        print("11111111111111 : course",course)
        # assume masquerading role, if applicable
        _course_masquerade, request.user = setup_masquerade(
            request,
            course_key,
        ...
        print("11111111111111 : _course_masquerade",_course_masquerade)
        # Record user activity for tracking progress towards a user's course goals (for mobile app)
        UserActivity.record_user_activity(
            request.user, usage_key.course_key, request=request, only_if_mobile_app=True
        )
        # get the block, which verifies whether the user has access to the block
        recheck_access = request.GET.get('recheck_access') == '1'
        print("11111111111111 : recheck_access",recheck_access)
        block, _ = get_block_by_usage_id(
            request,
            str(course_key),
            str(usage_key),
        ...
        print("11111111111111 : block",block)
        ...
        # Some content gating happens only at the Sequence level
        ancestor_sequence_block = enclosing_sequence_for_gating_checks(block)
        if ancestor_sequence_block:
            context_masq = {'specific_masquerade': is_masquerading_as_specific_student(request.user, course_key)}
            if ancestor_sequence_block.descendants_are_gated(context_masq):
                return redirect(
                    reverse(
                        'render_xblock_particular',
                        kwargs={'usage_key_string': str(ancestor_sequence_block.location)}
                    )
                )
        ...
        print("11111111111111 : context", context)
        try:
            context, student_view_context = RenderXBlockStarted.run_filter(
                context=context, student_view_context=student_view_context
            )
        except RenderXBlockStarted.PreventXBlockBlockRender as exc:
            log.info("11111111111111 Halted rendering block %s. Reason: %s", usage_key_string, exc.message)
            return render_500(request)
        except RenderXBlockStarted.RenderCustomResponse as exc:
            log.info("11111111111111 Rendering custom exception for block %s. Reason: %s", usage_key_string, exc.message)
            context.update({
                'fragment': Fragment(exc.response)
            })
            return render_to_response('courseware/courseware-chromeless.html', context, request=request)
